# 이돈섭 (Lee Don Sub)

> **상수도 디지털 트랜스포메이션을 1인이 풀스택으로 끝내는 도메인 × 기술 하이브리드 엔지니어**
> 자동제어 16년 · 마이크로웨이브 11년 · 위플랫 AI연구소에서 누수탐지 / 스마트캡슐 / AR / 엣지 AI까지 한 사이클을 혼자 닫는다

㈜위플랫 AI연구소 차장 (2025년~)

---

## 정체성

| 축 | 내용 |
|---|---|
| **도메인** | 누수음 탐지 / 배관 내부 진단 / 관망 분석 / 상수도 시스템 디지털화 |
| **기술** | 회로 · 펌웨어 · 기계설계 · Python 분석 · Jetson 엣지 · Unity AR · 로컬 LLM |
| **사업화** | 정부 R&D 30+ 과제 (TIPS / D-유니콘 / OI / K-water 등) 기획 · 수주 · 실행 |
| **글로벌** | 태국 납품 완료 · IBS 사업 진행 중 · 5개국 파이프라인 |
| **AI 활용** | Claude 페어 / Colab / 로컬 LLM — AI를 도구가 아니라 인프라로 |

---

## 일하는 방식

- **한 사이클 완결** — 기획서 → 수주 → 회로 / 펌웨어 설계 → 분석 SW → 현장 설치까지 1인이 닫음
- **AI를 인프라로** — PPT / BOM / 리포트 자동생성, 분석 워크플로우 Python 화, 엣지 LLM 운영
- **도메인이 본업** — 자동제어 16년 + 수도 / 마이크로웨이브 산업 깊이가 나머지 모든 기술의 방향타

---

## 전문 분야 (직접 수행)

- 자동제어 시스템 설계 · **PLC (LS산전)** / **HMI (M2I, LS산전)** 프로그래밍
- 자동화 / 전기 판넬 제작 · 시운전 (현장)
- 전기설계 (S마크 기준) — BOM · 계통도 · 상세회로도 (**SolidWorks Electrical**)
- 3D 외함 / 내판 — 제어반 / 판넬 (SolidWorks)
- 기구 보조 설계 — 제품 / 장비 외형 · 프레임 (SolidWorks)
- **아날로그 회로 설계** — 압전소자 센서 보드 / 누수음 신호 처리 (저잡음 / OPAMP)
- 누수음 탐지 · 신호 분석
- **마이크로웨이브 응용 설비** (1.1~3kW급, 11년) — 마그네트론 · 도파관 · HV Power Supply
- 해외 소형 정수장 염소 자동투입 장치 등 PLC / HMI 납품

## 설계 도구

![KiCad](https://img.shields.io/badge/KiCad-314CB0?style=flat&logo=kicad&logoColor=white)
![FreeCAD](https://img.shields.io/badge/FreeCAD-418FDE?style=flat&logo=freecad&logoColor=white)
![SolidWorks](https://img.shields.io/badge/SolidWorks-FF0000?style=flat&logo=dassaultsystemes&logoColor=white)
![SolidWorks Electrical](https://img.shields.io/badge/SolidWorks%20Electrical-FF6347?style=flat&logo=dassaultsystemes&logoColor=white)

- **전자회로 / PCB 분석** — KiCad
- **물성 / 구조 분석 (FEM)** — FreeCAD
- **기구 / 외함 설계** — SolidWorks
- **전기 회로도 / 계통도** — SolidWorks Electrical

## 주 사용 프로그램

![Unity](https://img.shields.io/badge/Unity-000000?style=flat&logo=unity&logoColor=white)
![Arduino IDE](https://img.shields.io/badge/Arduino%20IDE-00979D?style=flat&logo=arduino&logoColor=white)
![Android Studio](https://img.shields.io/badge/Android%20Studio-3DDC84?style=flat&logo=androidstudio&logoColor=white)
![QGIS](https://img.shields.io/badge/QGIS-589632?style=flat&logo=qgis&logoColor=white)

- **AR / 3D** — Unity
- **펌웨어 / 임베디드** — Arduino IDE
- **모바일 / 앱** — Android Studio
- **GIS / 관망 분석** — QGIS

## AI 협업으로 다루는 영역

![Python](https://img.shields.io/badge/Python-3776AB?style=flat&logo=python&logoColor=white)
![C](https://img.shields.io/badge/C-A8B9CC?style=flat&logo=c&logoColor=black)
![C++](https://img.shields.io/badge/C++-00599C?style=flat&logo=cplusplus&logoColor=white)
![TypeScript](https://img.shields.io/badge/TypeScript-3178C6?style=flat&logo=typescript&logoColor=white)
![ESP32](https://img.shields.io/badge/ESP32-E7352C?style=flat&logo=espressif&logoColor=white)
![Raspberry Pi](https://img.shields.io/badge/Raspberry%20Pi-A22846?style=flat&logo=raspberrypi&logoColor=white)
![NVIDIA Jetson](https://img.shields.io/badge/NVIDIA%20Jetson-76B900?style=flat&logo=nvidia&logoColor=white)
![Linux](https://img.shields.io/badge/Linux-FCC624?style=flat&logo=linux&logoColor=black)

데이터 분석 · GIS 자동화 · AR 스크립팅 · 펌웨어 / 임베디드 · 엣지 LLM 운영

## 주력 AI 도구

![Claude](https://img.shields.io/badge/Claude-D97757?style=flat&logo=anthropic&logoColor=white)
![ChatGPT](https://img.shields.io/badge/ChatGPT-74AA9C?style=flat&logo=openai&logoColor=white)
![Google Colab](https://img.shields.io/badge/Colab-F9AB00?style=flat&logo=googlecolab&logoColor=white)

> Claude Code 페어 프로그래밍 + Colab으로 모델 실험 → Raspberry Pi 로컬 LLM 운영

---

## 경력 타임라인

| 기간 | 회사 | 직책 | 주력 |
|---|---|---|---|
| **2025 ~ 현재** | **㈜위플랫 AI연구소** | **차장** | 누수탐지 R&D · 스마트캡슐 · 펌웨어 · AI |
| 2023 ~ 2024 | ㈜글래스돔 코리아 | 매니저 | PCF 모니터링 구축 / 현장 감독 |
| 2022 ~ 2023 | ㈜한테크 | 과장 | 전기판넬 제작 · 시운전 |
| 2020 ~ 2022 | ㈜세종이엠씨 | 차장 | MW 응용 설비 / 자동화 판넬 |
| 2015 ~ 2020 | ㈜파워써플라이 | 차장 | MW 응용 설비 / 자동화 판넬 |
| 2009 ~ 2015 | ㈜유림이엔지 | 사원 | MW 응용 설비 / 자동화 판넬 |

> **자동제어 트랙 16년 일관 — 5개 회사를 거치며 사원 → 차장 → 매니저 → AI연구소 차장**

---

## 주요 프로젝트 / 납품 이력

### ㈜위플랫 AI연구소 (2025 ~) — 상수도 디지털 트랜스포메이션 R&D

**진행 중**
- 상수도배관 증강현실 (Unity 기반 매설 배관 시각화)
- 통합관리시스템 (Jetson AGX Orin 엣지 디바이스)
- 스마트캡슐 R&D (배관 내부 영상 / 음향 수집 캡슐, 회수장치, 분해조립 지그)
- QGIS 상수도관망 분석기 (Python)
- Raspberry Pi 엣지 서버 (로컬 LLM)
- 정부 R&D — 딥테크 TIPS / 포스트 TIPS / OI 사업

**해외 사업**
- 태국 붕클라 (완료) — 소형 정수장 염소 자동투입 장치 PLC / HMI
- IBS 사업 (진행 중) — 해외 현장 시스템 개발
- 파이프라인 — 키르기스스탄 · 멕시코 · 탄자니아 · 미국 · 인도네시아

### ㈜글래스돔 코리아 (2023 ~ 2024) — PCF (Product Carbon Footprint) 모니터링 시스템 구축

- **조일알미늄 · 롯데알미늄** 탄소발자국 모니터링 시스템 구축 공사
- **하도급사 TSENG 관리** · 현장 감독
- **PLC 기반 DAQ (데이터 수집)** 직접 수행
- 탄소국경조정(CBAM) 대응 인프라 구축 흐름

### ㈜한테크 (2022 ~ 2023) — 자동화 판넬 제작 / 시운전

- 전기판넬 제작 및 현장 시운전

### ㈜유림이엔지 → ㈜파워써플라이 → ㈜세종이엠씨 (2009 ~ 2022, 13년) — 마이크로웨이브 응용 설비

- **1.1 ~ 3kW급 마이크로웨이브 시스템** — 마그네트론 · 도파관 (WAVE GUIDE LAUNCHER) · HV Power Supply · PWM 전력제어
- **자동화 판넬** 설계 / 제작 / 시운전
- **178개+ 고객사**:
  - 반도체 — 삼성전자 · 세메스 · 야스 · 새로닉스 · 비전세미콘 · 아바코
  - 정부 연구소 — 국가핵융합연 · 한국원자력연 · 한국에너지기술연 · 한국화학연 · 한국전자통신연 · 나노종합기술원
  - 플라즈마 / 진공 — 플라즈마 코리아 · 클린팩터스 · 페스코 · 퓨어플라텍
  - 대기업 — 현대자동차 · 현대제철 · 에코프로
  - 해외 / 학계 — KAUST (사우디) · 인하대 · 부경대 · 충남대 등

---

## 학력

- 한밭대학교 전자과 편입 졸업 (2013)
- 충북인력개발원 전기과 졸업 (2010)

## 자격증

- 전기기사
- 전기공사기사
- EMS (에너지관리시스템) 고급 과정 수료 — 에너지관리공단

---

> "무언가를 배울 수 있는 환경을 선호하며, 계획을 세워 책임감 있게 일을 끝내는 것"
